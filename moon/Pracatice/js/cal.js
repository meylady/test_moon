   ...
   this.renderCalendar =function(){
    ...
    //  console.log(r_date[1]);
      ...
      if(sClass == 'date-cell not-this-month ' || sClass == 'date-cell not-this-month sun'||sClass == 'date-cell not-this-month sat' ){
        arrTable.push('<td id="'+oStartDt.getFullYear()+'-' +(oStartDt.getMonth()+1)+ '-' +oStartDt.getDate()+ '" class="lastDate" class="'+sClass+'" >' + oStartDt.getDate() + '</td>');
      }
      else {
        if(r_date[0]==oStartDt.getFullYear() && r_date[1]==(oStartDt.getMonth()+1) &&r_date[2]== oStartDt.getDate() ){  //현재날짜와 일치할경우
            arrTable.push('<td id="'+oStartDt.getFullYear()+'-' +(oStartDt.getMonth()+1)+ '-' +oStartDt.getDate()+ '" class="rDate" class="'+sClass+'" >' + oStartDt.getDate() + '</td>');
        }
        else{
          arrTable.push('<td id="'+oStartDt.getFullYear()+'-'+(oStartDt.getMonth()+1)+ '-' +oStartDt.getDate()+ '" class="'+sClass+'"  >' + oStartDt.getDate() + '</td>');
        }
      }
...
$('td').click(function() {
  ...
  if(id != id_chk){
    if(id_chk!="0000-00-00"){
      ...
      else if(c_date[1]!=id_date[1]){ //지난달이던가 다음달이던가
          $('#'+id_chk+'').css("background-color","hsla(5, 7%, 70%, 0.3)");
          console.log(c_date[1]+1);
      }
      else{
